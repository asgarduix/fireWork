...
function printBtn() {
	...
	if ('1' === param.rdoMode) {//立即執行
		const url = "../../rin1206api/printrin1206report";
		const res = ajaxPostByJsonParam(url, parJson, true);
		if('000' === res.status){
			alert("列印完畢!!!");
		}
		
	}else if('2' === param.rdoMode){//排程執行
		const url = "../../rin1206api/insertscheduleforrin1206report";
		const result = ajaxPostByJsonParam(url, parJson, true);
