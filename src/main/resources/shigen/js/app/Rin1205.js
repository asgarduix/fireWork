...
function fake1205(){
	$('#txtrisk_no').val('8151000010');
	$('#txttreaty_year').val('2021');
	$('#dta').val('2021/10/10');
}
...
function btnQueryRin1205(){
	...
	if(checkIsNullSpace($('#dta').val())){
		message = message + "有效日期不可為空白!"+nextRow;
	}
	if(!checkDateFormat($('#dta').val())){
		message = message + "有效日期格式需為yyyy/mm/dd!"+nextRow;
	}
	...
	let res = ajaxPostByJsonParam("../../rin1205api/query1205list", parJson, false);
	
	
	//顯示
	if('000' === res.status){
		
		loadData("table1205", res.data, {type:"dataCount", value:15})
		$('#btnSaveDetail').show();
...
function saveDetail(){
	...
	for(let i = 0; i < table1205.getData().length; i++){
	    ...
	    let thisData = table1205.getData()[i];	    
		let shareAmtSum = 0;
		
		for(let j = 0; j < thisData.rin1205SubVo.length; j++){
			shareAmtSum = shareAmtSum*1 + thisData.rin1205SubVo[j].numshare_amt_i*1
		}
	   
	    //檢核合約分保總額是否等於總保額(總保額-自留保額-臨分保額-各個合約分出保額 是否為0)
	    let moneyCal = thisData.numamt - thisData.numshare_amt_0 - thisData.sum_amt_1 - shareAmtSum
	...
	if(!checkIsNullSpace(msg)){
		confirm("現行資料有合約分保總額不等於總保額狀況，請確認是否仍要將資料寫入"+"\n"+msg+"確認更正資料請按[確定]，重新調整資料請按[取消]");
		return;
	}
...
let columns1205 = [
	...
	[ "numamt", "總保額", "number",{editable:()=>{return false},minWidth:wordTitle*3+colwidthDe}],
	[ "numprem", "總保費", "number",{editable:false,minWidth:wordTitle*3+colwidthDe}],
	[ "numprem_rate", "保單費率", "display", {minWidth:wordTitle*4+colwidthDe}],
	[ "numshare_prem_rate", "分保費率", "display", {minWidth:wordTitle*4+colwidthDe}],
	[ "numlimit", "保單限額", "display", {minWidth:wordTitle*4+colwidthDe}],
	[ "numshare_amt_0", "自留保額", "input", {minWidth:wordTitle*4+colwidthDe}],
	[ "numshare_prem_0", "自留保費", "input", {minWidth:wordTitle*4+colwidthDe}],
	[ "sum_amt_1", "臨分保額", "display", {minWidth:wordTitle*4+colwidthDe}],
	[ "numamt_ear_1", "地震自留", "display", {minWidth:wordTitle*4+colwidthDe}],
	[ "numamt_typ_1", "颱風自留", "display", {minWidth:wordTitle*4+colwidthDe}],
	[ "numamt_ear_2", "地震臨分", "display", {minWidth:wordTitle*4+colwidthDe}],
	[ "numamt_typ_2", "颱風臨分", "display", {minWidth:wordTitle*4+colwidthDe}],
...
//                    elementsChangeClass(row.getElement(), "add", "table-editable")
            ...
            let subColumns = [
                ["txttreaty_no_i", "合約代號", "display",],
                ["numshare_amt_i", "分保額", "input",],
                ["numshare_prem_i", "分保費", "input",],
            ]
