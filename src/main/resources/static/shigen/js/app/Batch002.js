...
function stop() {
	...
	let batchID = tableBatch002.getSelectedRows()[0].getData().batchid;

	// Rin1204 允許在"進行中"的狀態，強制停止
	if (processstatus == '0' || (batchID == 'Rin1204' && processstatus == '1')) {
...
function reload() {
	...
	if(checkIsNullSpace($('#timeReload').val().trim())){
		alert("更新間隔頻率為必填!");
		return;
	}
