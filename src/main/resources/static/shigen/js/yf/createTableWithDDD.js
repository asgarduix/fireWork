...
function createTableWithDDD(tableParams, tabledata, destination, showBtn){

  // const apiData = crtDdlLoadDDLIINSCLSType2Val();
  // let myMap = {};
  // for (let i=0; i< apiData.length; i++){
  //   myMap[apiData[i].value] = apiData[i].text;
  // }

  // const apiData1 = crtDdlLoadDDLIINSKind_501Type2Val();
  // let myMap1 = {};
  // for (let i=0; i< apiData1.length; i++){
  //   myMap1[apiData1[i].value] = apiData1[i].text;
  // }
  

  // const apiData2 = crtDdlLoadDDLNCMNpInstypeType2Val();
  // let myMap2 = {};
  // for (let i=0; i< apiData2.length; i++){
  //   myMap2[apiData2[i].value] = apiData2[i].text;
  // }
    ...
    //{title: "Ins", field: "insType", editor: "select", editorParams: {values:myMap}, editable: isEditable, headerClick: handleHeaderClick, cellEdited: handleCellEdited, cellEditing: handleCellEditing , cellEditCancelled: handleEditCancelled},
    //{title: "Ins1", field: "insType1", editor: "select", editorParams: insFunc1, editable: isEditable, headerClick: handleHeaderClick, cellEdited: handleCellEdited1, cellEditing: handleCellEditing , cellEditCancelled: handleEditCancelled},
    //{title: "Ins2", field: "insType2", editor: "select", editorParams: insFunc2, editable: isEditable, headerClick: handleHeaderClick, cellEditing: handleCellEditing , cellEditCancelled: handleEditCancelled}
  ...
  for (let i=0; i < tableParams.length; i++){
    if (tableParams[i][2] === "select" && (typeof tableParams[i][3]) === "object"){
      columns.push({
	  ...
	  })} else if (tableParams[i][2] === "select" && (typeof tableParams[i][3]) === "function"){
      columns.push({
        title: tableParams[i][1],
		    field: tableParams[i][0],
        editor: `${tableParams[i][2]}`,
        editorParams: function(cell){
          if (tableParams[i][5] === true){
            let type = tableParams[i-1][0];
            let d = cell.getRow().getData()[type];
            let apiData1 = tableParams[i][3]("Y", d);
            let myMap1 = {};
            for (let i=0; i< apiData1.length; i++){
              myMap1[apiData1[i].value] = apiData1[i].text;
            }
            return { values:myMap1 }
              
          } else {
            let apiData = tableParams[i][3]()
            let myMap = {};
            for (let i=0; i< apiData.length; i++){
              myMap[apiData[i].value] = apiData[i].text;
            }
...
// let tabulatorCells = document.getElementsByClassName("tabulator-cell");
// console.log(tabulatorCells);
// for (let i = 0; i < tabulatorCells.length; i++){
//   $(arrow).appendTo(tabulatorCells[i]);
