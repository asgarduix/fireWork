function createTable(tableId, columns, tableConfigs, tableRelatedBtns, alertConfig) {
	...
	<div>
		...
		<div>頁</div>
            ...
            $(`#${tableId}-pagination .dpPerPageNum`).empty()[0].innerHTML = perPageSizeOptsStr;
    ...
    function createTableBtns(btnConfig) {
        ...
        $(btnConfig.position)[0].appendChild(btn);
    }
    ...
    function add(tableId, getDefaultData) {
        ...
        table.deselectRow()
        elementsChangeClass(`#${tableId}-add, #${tableId}-edit, #${tableId}-copy, #${tableId}-del`, "add", "btn-hide")
        elementsChangeClass(`#${tableId}-cancel, #${tableId}-save`, "remove", "btn-hide")
        table.clearHistory()
        let newDataId = "selectedRow"
        table.editableRowIndex = newDataId
        let dataCount = table.getDataCount()

        if (dataCount > 0) {
            table.getRows()[0].update({ id: "top" })
            table.scrollToRow("top", "top", false)
                .then(() => {
                    addNewData(tableId)
                    setTimeout(() => {
                        $(".ui-datepicker").css("display", "none")
                    }, 0)
                    table.getRow("top").update({ id: "" })
                })
        } else {
            addNewData(tableId)
            setTimeout(() => {
        ...
        function addNewData(tableId) {
            let table = Tabulator.prototype.findTable(`#${tableId}`)[0];
            table.addData([{...defaultData, _edit: true, id: newDataId }], true).then(() => {
    ...
    function cancel(tableId) {
        ...
        let editableDatas = table.getRows().filter(row => { return row.getData()._edit })
    ...
    function edit(tableId) {
        ...
        selectRows.forEach(row => {
            row.update({ _edit: true, id: "selectedRow" })
    ...
    function copy(tableId) {
        ...
        table.scrollToRow("top", "top", false)
            .then(() => {
                table.addData([{...rowsDataArry[0], _edit: true, id: newDataId }], true);
    ...
    function del(tableId, delApi) {
        ...
        if (delConfirm) {
            ...
            if (apiRespond.isSuccess) {
                ...
                // 	row.delete()
                // });
                let remainData = table.getRows().reduce((arry, row) => {
                    if (row.isSelected()) { return arry } else { return [...arry, row.getData()] }
                }, []);
                table.clearData();
                table.setData(remainData);
    ...
    function save(tableId, nullSpaceCheck, addSaveApi, editSaveApi, rules) {
        ...
        let errMsg = inputCells.reduce((arry, cell) => {
            ...
            let msg = ""
            elementsChangeClass(el, "remove", "table-error-null table-error-custom")
...
//        }
    ...
    function tableAlert(mode) {
        ...
        switch (alertType) {
            case "alert":
                mode === "success" || alert(table.errMsg)
                break
            case "dom":
                if (mode === "success") {
    ...
    return table.setData(data).then(() => {
        ...
        if (heightType === "height") {
            ...
            if (data.length > displayRowsNum) {
                ...
                // TODO 目前某些情況下，因不明原因導致 headerHeight, rowHeight, newHeight 都是 0 ，故在此先寫死若為0之情形都設260
