...
public class SysFuncUiButtonRepository {
	 ...
	 * 
	 */
	public List<SysFuncUiButton> queryByModelBetweenSize(final SysFuncUiButton model, String orderByColNm1, String ascOrDesc, int pageSize, int pageNum)
			throws Exception {
	...
	public int insertList(final List<SysFuncUiButton> modelList)
		...
		try {
			SysFuncUiButtonMapper mapper = sqlSession
					.getMapper(SysFuncUiButtonMapper.class);
...
//            log.debug(returnList == null ? "returnList is null" : "筆數:" + returnList.size());
	...
	*/
	...
	//	int result = 0;
	//	try {
	//		SysFuncUiButtonMapper mapper = session.getMapper(SysFuncUiButtonMapper.class);
