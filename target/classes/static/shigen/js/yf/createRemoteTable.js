...
function createRemoteTable(colDefines, destination, ajaxUrl, ajaxParams, btnmethods, showNo, statusBarBtnMethods, idColumnName, nonEditableParams) {
	let currentPage = 1; // to keep track of the current page the table is on to carry out pagination after table.setData();
	let dataSize; //總共幾筆
	let pageSize; //總共幾頁
	var col = {};
	...
	for (var i = 0; i < colDefines.length; i++) {
		...
		} else if (param[0] === "CorrectButton") {
			defineObjTmp.push("");

			columns.push({
				title: param[0].charAt(0).toUpperCase() + param[0].slice(1),
				field: param[0],
				formatter: formatterCorrectButton,
				cellClick: correctButtonClick,
				headerSort: false,
				resizable: false,
				download: false,
				visible: param[3]
			});
		} else if (param[0] === "QueryButton") {
			defineObjTmp.push("");

			columns.push({
				title: param[0].charAt(0).toUpperCase() + param[0].slice(1),
				field: param[0],
				formatter: formatterQueryCorrectButton,
				cellClick: queryButtonClick,
				headerSort: false,
				resizable: false,
				download: false,
				visible: param[3]
	...
	function formatterCorrectButton(cell, formatterParams) {
		return "<div name='correctBtn' class='tabulator-correctBtn btn'>更正</div>";
	}

	//create query button
	function formatterQueryCorrectButton(cell, formatterParams) {
		return "<div name='queryBtn' class='tabulator-queryBtn btn'>查詢</div>";
	}
	 ...
	 *針對cacnel中使用的共用程式
	*/
	async function proc4NewCloneFlow(cell) {
		...
		if (window.confirm("確定要刪除此筆資料嗎?")) {
			try {
				let result = await btnmethods.delete(e, cell);
				//當刪除完資料後，再重新查詢一次
				table.setData();
			} catch (err) {
				console.log("callback function(edit) failed");
	...
	function correctButtonClick(e, cell) {
		try {
			btnmethods.corr(e, cell);
		} catch (err) {
			console.log("callback function(correct) failed");
			console.log(err);
		}
	}

	function queryButtonClick(e, cell) {
		try {
			btnmethods.query(e, cell);
		} catch (err) {
			console.log("callback function(query) failed");
			console.log(err);
		}
	}
	 ...
	 * @param {*} cell
	 */
	async function saveButtonClick(e, cell) {
		if (!cell.getRow().isSelected()) {
			return;
		}
		currentPage = cell.getTable().getPage();
		...
		try {
			...
			console.log("call api to write to db")
			let result = await btnmethods.update(e, cell);
			//當在資料庫完新增完新的一筆資料後，再重新查詢一次
			table.setData().then( data => {
				//資料塞完之後再換到正確的頁面(第幾頁)
				try{
...
//		console.log("editor_msg-row_index:" + index + ", judge_modified:" + isModified + ", data:" + JSON.stringify(modifyRes));
	...
	//delete function
	//TODO
	async function deleteButtonClick(e, cell) {
		//Can use prompt to make them confirm the name
		if (window.confirm("確定要刪除嗎?")) {
			currentPage = cell.getTable().getPage();
			const isLastPage = currentPage === pageSize;
			const isTopRow = cell.getRow().getPosition() === 0;
			try {
				//cell.getRow().delete()
				let result = await btnmethods.delete(e, cell);
				//當刪除完資料後，再重新查詢一次
	...
	const paramsData = {isOid: "1000019007"};
	//Build Tabulator
	...
	//////////////////////////////////////////////////
		...
		rowDeleted: function(row){
			console.log("tabulator row deleted")
			currentPage = table.getPage();
			table.setData();

			//after data is set call table.setPage(currentPage);
			setTimeout(()=> {
				table.setPage(currentPage);
			}, 100);
		},
		*/
		pageLoaded: function(pageno) {
			...
			document.getElementById(curPageNumId).value = pageno;
			//$(currentPageDiv).remove();
			//createCurrentPage();
		...
		dataLoaded: function(data){
			//will cause table to keep loading new data
			//setTimeout(()=> table.setPage(currentPage));
		},
		/*
	...
	$(`${destination}-reactivityAdd`).on("click", function() {
		...
		if ( idColumnName ){
			pushData = { [idColumnName]: uuidv4()}
		}else{
			pushData = { uniqueId: uuidv4()}
		}
		...
		//pushData = {...pushData, oid: 123456};
		table.addRow(pushData, true).then( row =>{
			let cell = row.getCells()[0];
			switchVisibility(cell);
			change2EditStatus(cell);
		})
	...
	//@author IsaHuang 2021/07/27 - 批改的「修改被保險人資料按鈕」需要的
	$(`${destination}-reactivityAddUp`).prependTo(pagination);
	setTimeout(function() { $(`${destination}-reactivityAddUp`).addClass("addRowBtnUp") }, 0.1)
	...
	function createDataNum() {
		...
		let dataNumClassPrefix = destination.slice(1);
		dataNum.classList.add(`${dataNumClassPrefix}-dataNum`)
	...
	function createPageNum() {
		...
		let pageNumClassPrefix = destination.slice(1);
		pageNum.classList.add(`${pageNumClassPrefix}-pageNum`);
	...
	//1.如果是在table以外新增，點選新增後 => 呼叫api => 寫入資料庫 => 重查
	//2.如果是在table內新增，點選確定後 => 呼叫api => 寫入資料庫 => 重查
