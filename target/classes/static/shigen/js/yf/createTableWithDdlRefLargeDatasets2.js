...
function refreshPage(tableId, stayOnCurrentPage = true) {
	const fetchedTable = fetchTabulator(tableId);
	let { url, param } = fetchedTable.queryParams;
	let curPageNum;
	...
	//$(`${tableId}-pagination`).find(".totalDataNum").text(`總共  ${dataLength} 筆`); //dataLength will come from db
	//$(`${tableId}-pagination`).find(".totalPageNum").text(`總共  ${Math.ceil(dataLength / dpPerPg)} 頁`);
...
function createTableddlrefLDS2(colDefines, destination, btnmethods, showNo, statusBarBtnMethods, nonEditableParams) {
	var col = {};
	let currentLoadData = [];
	...
	*/
	...
	function deleteButtonClick(e, cell) {
		let dataLength = cell.getTable().getData().length;
		let rowIndex = cell.getRow().getPosition();
		//Can use prompt to make them confirm the name
		//if (window.confirm("Delete Data No." + (rowIndex + 1))) {
			cell.getRow().delete();
			try {
				btnmethods.delete(e, cell, rowIndex,dataLength);
			} catch (err) {
				console.log("callback function(delete) failed");
				console.log(err);
			}
	...
	var table = new Tabulator(`${destination}`, {
		...
		dataLoaded: function(data) {
			setTimeout(() => ablePageNavButtons(), 100);
			currentLoadData = data;
			if (data !== []) {
				createPageNavNumButtons(data);
			}
	...
	$(`${destination}-reactivityAdd`).on("click", function() {
		...
		if ( totalDataPoints % dpPerPg !== 0){
			...
			$(`${destination}-pagination`).find(".totalPageNum").text(`總共  ${totalPageNum + 1} 頁`);
			table.replaceData([
	...
	*/
	...
	function createDpPerPage() {
		...
		$(dpPerPageDiv).on("submit", function(event) {
			...
			/*
			let dataNumInnerText = $(".totalDataNum")[0].innerText;
			...
			if ( pgNum <= maxPossiblePageNum && pgNum > 0 && dpPerPg > 0)  {
				...
				//$(pageNumDiv).remove();
				//createPageNum()
				$(".totalPageNum").text(`總共  ${Math.ceil(totalDataPoints / dpPerPg)} 頁`);
				//let dataLength = data.totalDatapoints
				//$(".totalPageNum").text(`總共  ${Math.ceil(dataLength / dpPerPg)} 頁`);
		...
		$(goBtn1).html("go");
		$(goBtn1).addClass("tabulator-goBtn");
	...
	function createCurrentPage() {
		...
		$(currentPageDiv).on("submit", function(event) {
			event.preventDefault();
			/*
			//let pageNumInnerText = $(".totalPageNum")[0].innerText;
		...
		$(goBtn2).html("go");
		$(goBtn2).addClass("tabulator-goBtn");
	...
	*/
	...
	function ablePageNavButtons() {
		$('button[title="First Page"]').removeAttr("disabled");
		$('button[title="Prev Page"]').removeAttr("disabled");
		$('button[title="Next Page"]').removeAttr("disabled");
		$('button[title="Last Page"]').removeAttr("disabled");
		//$('button[title="First Page"]').attr("disabled", false);
		//$('button[title="Prev Page"]').attr("disabled", false);
		//$('button[title="Next Page"]').attr("disabled", false);
		//$('button[title="Last Page"]').attr("disabled", false);
	}

	function createPageNavNumButtons() {
		//console.log("createpagenavnumbuttons")
		let numBtnsDiv = document.createElement("div");
		$(numBtnsDiv).addClass("tabulator-numBtnsDiv");
		$(numBtnsDiv).css({ width: "65px", height: "10px", display: "inline-block" });
		let prevPageBtn = $(`${destination}`).find('button[title="Prev Page"]');
		$(numBtnsDiv).insertAfter(prevPageBtn);
		//TODO
		for (let i = 1; i < 6; i++){
			let numBtn = document.createElement("button");
			$(numBtn).text(i);
			$(numBtn).addClass("tabulator-numBtn");
			$(numBtn).css({width:"10px", height: "10px"});
			$(numBtn).appendTo(numBtnsDiv);
			$(numBtn).on("click", function(){
				$(`${destination}-pagination`).find(".curPageNum").val(i)
			})
		}
	}
